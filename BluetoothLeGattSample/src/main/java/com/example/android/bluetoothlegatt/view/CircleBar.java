...
public class CircleBar extends View {
	...
	private float mCount;// 为了达到数字增加效果而添加的变量，他和mText其实代表一个意思
	/************最大计数************/
	...
	private void init() {
		...
		mTextSize1 = dip2px(getContext(), 15);// 上下文字大小
	...
	protected void onDraw(Canvas canvas) {
		...
		/***********根据当前计数计算角度************/
		mProgressAni = MAX_ANGLE * (mCount / mMaxCount);
		...
		if (this.mType == 1) {// 第一个页面
			upText = "步数";
			downText = "目标:10000";
			middleText = String.valueOf((int)mCount);
...
//        Log.i(TAG, "onTouchEvent: ");
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (onCircleBarClickListener != null) {
                    onCircleBarClickListener.onClick();
                }
                break;
            default:
                break;
    ...
    public interface OnCircleBarClickListener {
        void onClick();
    }

    public void setOnCircleBarClickListener(OnCircleBarClickListener onCircleBarClickListener) {
        this.onCircleBarClickListener = onCircleBarClickListener;
    }

    private OnCircleBarClickListener onCircleBarClickListener;
